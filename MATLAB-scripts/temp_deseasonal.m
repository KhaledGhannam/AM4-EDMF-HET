...
era.t2m  = ncread('/net2/Zhihong.Tan/OBS/Datasets/cjs_ecmwf/era_interim_monthly.1980-2014.climo.nc', 't2m');
...
era_inp.t2m=NaN(288,180,12);

for i=1:12
era_inp.t2m (:,:,i)  = interp2(X, Y, squeeze(era.t2m_ext(:,:,i)),  Xp, Yp);
end
%----------

% ---read model data ----
am4_orig.t2m= squeeze(mean(reshape(ncread(f_orig, 't_ref'),288,180,12,[]),4))-273.2;
am4_ncep.t2m= squeeze(mean(reshape(ncread(f_ncep, 't_ref'),288,180,12,[]),4))-273.2;
am4_clasp.t2m=squeeze(mean(reshape(ncread(f_clasp, 't_ref'),288,180,12,[]),4))-273.2;
%----------------------------------

era_t2m_jja = squeeze(mean(era_inp.t2m(:,:,6:8),3,'omitnan'));
era_t2m_jf =  squeeze(mean(era_inp.t2m(:,:,1:2),3,'omitnan'));

am4_t2m_jja = squeeze(mean(am4_orig.t2m(:,:,6:8),3,'omitnan'));
am4_t2m_jf = squeeze(mean(am4_orig.t2m(:,:,1:2),3,'omitnan'));

ncep_t2m_jja = squeeze(mean(am4_ncep.t2m(:,:,6:8),3,'omitnan'));
ncep_t2m_jf = squeeze(mean(am4_ncep.t2m(:,:,1:2),3,'omitnan'));

clasp_t2m_jja = squeeze(mean(am4_clasp.t2m(:,:,6:8),3,'omitnan'));
clasp_t2m_jf =  squeeze(mean(am4_clasp.t2m(:,:,1:2),3,'omitnan'));
%-----------------------------------------------------
...
framem;
contourfm(lat,lon,a,[-3:0.1:3], 'LineStyle', 'none'); hold on;
...
title({'(a) T_{ref} (AM4-EDMF-HET minus ERA)', sprintf('(%s; BIAS=%5.2f; RMSE=%5.2f)',upper(sea),s.bias41,s.rmse41)},'FontSize',14);
%------------------------------------------------
...
title({'(b) T_{ref} (AM4-EDMF-HET minus AM4-Lock)', sprintf('(%s; DIFF=%5.2f)',upper(sea),s.bias42)},'FontSize',14);
%------------------------------------------------
...
title({'(c) T_{ref} (AM4-EDMF-HET minus AM4-EDMF)', sprintf('(%s; DIFF=%5.2f)',upper(sea),s.bias43)},'FontSize',14);
%------------------------------------------------
...
xticklabels({'Dec.','Mar.','Jun.',...
    'Sep.'})
ylabel('T_{ref}  Bias (^{\circ}C)','FontSize',16)
ylim([-2 2])
yticks([-2 -1 0 1 2])
...
plot1 = plot(1:12,xx_clasp - xx_era,'r-o','MarkerSize',6,'linewidth',1.25,'DisplayName','AM4-EDMF-HET - ERA');
hold on
plot2 = plot(1:12,xx_ncep - xx_era ,'b->','MarkerSize',6,'linewidth',1.25,'DisplayName','AM4-EDMF - ERA');
plot3 = plot(1:12,xx_am4 - xx_era,'g-s','MarkerSize',6,'linewidth',1.25,'DisplayName','AM4-Lock - ERA');
plot([1 12],[0 0],'k-','linewidth',1.25)
delete(hh);
hh=legend([plot1 plot2 plot3]);
set(hh,'Orientation','Vertical','Position',[0.80 0.63 0.195075760569109 0.122674420939227],...
    'box','off','Fontsize',16)
...
saveas(gcf,'Tref_deseasonal.png')
