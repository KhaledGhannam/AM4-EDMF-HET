...
p_am4_time=86400.*squeeze(ncread(f_orig, 'precip'));
p_ncep_time=86400.*squeeze(ncread(f_ncep, 'precip'));
p_clasp_time=86400.*squeeze(ncread(f_clasp, 'precip'));
...
%  clear hh1 pp1 hh2 pp2
    x_am4   = squeeze(p_am4_time(i,j,:));
    x_ncep  = squeeze(p_ncep_time(i,j,:));
    x_clasp = squeeze(p_clasp_time(i,j,:));
%
    [hh1,pp1]=ttest2(x_am4,x_clasp,'Vartype','unequal');
    [hh2,pp2]=ttest2(x_ncep,x_clasp,'Vartype','unequal');
...
[significantLat1, significantLon1] = find(mask1' == 1);

significantLat1 = lat(significantLat1);   % Get latitudes of significant points
significantLon 1= lon(significantLon1);   % Get longitudes of significant points

% Overlay significant points as gray dots
hold on;
scatterm(significantLat1, significantLon1, 6, [0.5 0.5 0.5], 'filled'); % Gray dots

%-------------------------------------
ax6=subplot('Position',p6); clear a; a=z4z3;
%a(land_frac'<0.4 & (a>-0.15 & a<0.15))=NaN;
%a(land_frac'<0.4 & a<-0.15)=a(land_frac'<0.4 & a<-0.15)+0.15;
%a(land_frac'<0.4 & a>+0.15)=a(land_frac'<0.4 & a>+0.15)-0.15;
%a(land_frac'>0.4 & (a>-0.05 & a<0.05))=NaN;
axesm('MapProjection',domap,'origin',[0,180]);
framem;
contourfm(lat,lon,a,[-1:0.1:1], 'LineStyle', 'none'); hold on;
contourm(latx,lonx,lm,'k','LineWidth',1);
box off; axis off;
caxis([-1 1]); colormap(ax6,bluewhitered(nn)); %freezeColors;
...
[significantLat, significantLon] = find(mask2' == 1);

significantLat = lat(significantLat);   % Get latitudes of significant points
significantLon = lon(significantLon);   % Get longitudes of significant points

% Overlay significant points as gray dots
hold on;
scatterm(significantLat, significantLon, 6, [0.5 0.5 0.5], 'filled'); % Gray dots

%-------------------------------------


cd('/home/Khaled.Ghannam/awg/xanadu_2022.08/james_ana_03152023/figures/')
saveas(gcf,'precip_annualmean.png')
