...
per='%';
% ---- read variables -----
tke_ncep = squeeze(ncread('/archive/Khaled.Ghannam/awg/xanadu_2022.08/james_data_03152023/1980_2014_ncep_zas/tke_han_lev.nc','tke_han'));

tke_clasp = squeeze(ncread('/archive/Khaled.Ghannam/awg/xanadu_2022.08/james_data_03152023/1980_2014_clasp_lnd_disp_lm_zas/tke_han_lev.nc','tke_han'));

tke_clasp_nolm = squeeze(ncread('/archive/Khaled.Ghannam/awg/xanadu_2022.08/james_data_03152023/1980_2014_clasp_lnd_disp_zas/tke_han_lev.nc','tke_han'));


pfull_clasp = squeeze(ncread('/archive/Khaled.Ghannam/awg/xanadu_2022.08/james_data_03152023/1980_2014_clasp_lnd_disp_lm_zas/tke_han_lev.nc','pfull'));


%----CAUTION: REMOVE THIS LINE TO PLOT TKE_CLASP INSTEAD OF NO LM ----
tke_clasp = tke_clasp_nolm;

%------- Kt vertical profiles zonal means (land only)-----
...
tke_clasp_lnd = 1.*tke_clasp; tke_clasp_lnd(xx<0.4)=NaN;
tke_ncep_lnd = 1.*tke_ncep; tke_ncep_lnd(xx<0.4)=NaN;

slice_us_clasp = squeeze(mean(tke_clasp_lnd(id1,id2,:,:),4,'omitnan'));
slice_us_ncep = squeeze(mean(tke_ncep_lnd(id1,id2,:,:),4,'omitnan'));

slice_us_clasp_jja = reshape(tke_clasp_lnd(id1,id2,:,:),length(id1),length(id2),size(tke_clasp_lnd,3),12,[]);
slice_us_clasp_jja = mean(mean(slice_us_clasp_jja(:,:,:,6:8,:),4,'omitnan'),5,'omitnan');

slice_us_ncep_jja = reshape(tke_ncep_lnd(id1,id2,:,:),length(id1),length(id2),size(tke_ncep_lnd,3),12,[]);
...
"ShowText",true,"LabelFormat","%0.1f", "FaceAlpha",1,'LabelSpacing',300);
...
h3.Ticks = [0 0.5 1 1.5];
h3.TickLabels = {'0','0.5','1','1.5'};
xticks([241.8750 260.6250 279.3750])
xticklabels({'120^{\circ}W','100^{\circ}W','80^{\circ}W'})
%xlabel('Longitude')
ylabel('Pressure (hPa)')
yticks([600 700 800 900])
yticklabels({'600','700','800','900'})
set(get(h3,'Title') ,'String','m^{2}/s^{2}');
title({'(a) SGS TKE (CONUS; 25N-50N)','(AM4-EDMF-HET)'},'FontSize',14);
...
h4.Ticks = [-0.06 -0.03 0 0.03 0.06];
h4.TickLabels = {'-0.06','-0.03','0','0.03','0.06'};
%xlabel('Longitude')
ylabel('Pressure (hPa)')
xticks([241.8750 260.6250 279.3750])
xticklabels({'120^{\circ}W','100^{\circ}W','80^{\circ}W'})
yticks([600 700 800 900])
yticklabels({'600','700','800','900'})
set(get(h4,'Title') ,'String','m^{2}/s^{2}');
title({'(b) SGS TKE (CONUS; 25N-50N)','(AM4-EDMF-HET minus AM4-EDMF)'},'FontSize',14);
...
slice_afr_clasp = squeeze(mean(tke_clasp_lnd(id1,id2,:,:),4,'omitnan'));
slice_afr_ncep = squeeze(mean(tke_ncep_lnd(id1,id2,:,:),4,'omitnan'));

slice_afr_clasp_jja = reshape(tke_clasp_lnd(id1,id2,:,:),length(id1),length(id2),size(tke_clasp_lnd,3),12,[]);
slice_afr_clasp_jja = mean(mean(slice_afr_clasp_jja(:,:,:,6:8,:),4,'omitnan'),5,'omitnan');
slice_afr_ncep_jja = reshape(tke_ncep_lnd(id1,id2,:,:),length(id1),length(id2),size(tke_ncep_lnd,3),12,[]);
...
h5.Ticks = [0 0.5 1 1.5];
h5.TickLabels = {'0','0.5','1','1.5'};
...
%xlabel('Longitude')
ylabel('Pressure (hPa)')
set(get(h5,'Title') ,'String','m^{2}/s^{2}');
title({'(c) SGS TKE (Tropical Africa; 10S-10N)','(AM4-EDMF-HET)'},'FontSize',14);
...
h6.Ticks = [-0.06 -0.03 0 0.03 0.06];
h6.TickLabels = {'-0.06','-0.03','0','0.03','0.06'};
%xlabel('Longitude')
ylabel('Pressure (hPa)')
xticks([15.6250 30.6250 45.6250])
xticklabels({'15^{\circ}E','30^{\circ}E','45^{\circ}E'})
yticks([600 700 800 900])
yticklabels({'600','700','800','900'})
set(get(h6,'Title') ,'String','m^{2}/s^{2}');
title({'(d) SGS TKE (Tropical Africa; 10S-10N)','(AM4-EDMF-HET minus AM4-EDMF)'},'FontSize',14);
...
slice_ama_clasp = squeeze(mean(tke_clasp_lnd(id1,id2,:,:),4,'omitnan'));
slice_ama_ncep = squeeze(mean(tke_ncep_lnd(id1,id2,:,:),4,'omitnan'));

slice_ama_clasp_jja = reshape(tke_clasp_lnd(id1,id2,:,:),length(id1),length(id2),size(tke_clasp_lnd,3),12,[]);
slice_ama_clasp_jja = mean(mean(slice_ama_clasp_jja(:,:,:,6:8,:),4,'omitnan'),5,'omitnan');
slice_ama_ncep_jja = reshape(tke_ncep_lnd(id1,id2,:,:),length(id1),length(id2),size(tke_ncep_lnd,3),12,[]);
...
contourf(lon(id1),flipud(pfull_clasp(end-max_h:end)),a', 5, 'LineStyle', '-',...
"ShowText",true,"LabelFormat","%0.1f", "FaceAlpha",1,'LabelSpacing',200);
...
h7.Ticks = [0 0.5 1 1.5];
h7.TickLabels = {'0','0.5','1','1.5'};
%xlabel('Longitude')
ylabel('Pressure (hPa)')
...
yticks([600 700 800 900])
yticklabels({'600','700','800','900'})
set(get(h7,'Title') ,'String','m^{2}/s^{2}');
title({'(e) SGS TKE (Amazon; 10S-10N)','(AM4-EDMF-HET)'},'FontSize',14);


ax8=subplot('Position',p8); clear a; a=diff_clasp_ncep_jja;
contourf(lon(id1),flipud(pfull_clasp(end-max_h:end)),a', 7, 'LineStyle', '-',...
"ShowText",true,"LabelFormat","%0.2f", "FaceAlpha",1,'LabelSpacing',300);
...
h8=colorbar('Location','eastoutside','Position',pos8,'FontSize',16);
h8.Ticks = [-0.06 -0.03 0 0.03 0.06];
h8.TickLabels = {'-0.06','-0.03','0','0.03','0.06'};
%xlabel('Longitude')
ylabel('Pressure (hPa)')
xticks([281.8750 300.6250 319.3750])
xticklabels({'80^{\circ}W','60^{\circ}W','40^{\circ}W'})
yticks([600 700 800 900])
yticklabels({'600','700','800','900'})
set(get(h8,'Title') ,'String','m^{2}/s^{2}');
title({'(f) SGS TKE (Amazon; 10S-10N)','(AM4-EDMF-HET minus AM4-EDMF)'},'FontSize',14);
...
slice_esia_clasp = squeeze(mean(tke_clasp_lnd(id1,id2,:,:),4,'omitnan'));
slice_esia_ncep = squeeze(mean(tke_ncep_lnd(id1,id2,:,:),4,'omitnan'));

slice_esia_clasp_jja = reshape(tke_clasp_lnd(id1,id2,:,:),length(id1),length(id2),size(tke_clasp_lnd,3),12,[]);
slice_esia_clasp_jja = mean(mean(slice_esia_clasp_jja(:,:,:,6:8,:),4,'omitnan'),5,'omitnan');
slice_esia_ncep_jja = reshape(tke_ncep_lnd(id1,id2,:,:),length(id1),length(id2),size(tke_ncep_lnd,3),12,[]);
...
h9.Ticks = [0 0.5 1 1.5];
h9.TickLabels = {'0','0.5','1','1.5'};
%xlim([75 115])
...
xlabel('Longitude')
ylabel('Pressure (hPa)')
set(get(h9,'Title') ,'String','m^{2}/s^{2}');
title({'(g) SGS TKE (East Asia; 20N-40N)','(AM4-EDMF-HET)'},'FontSize',14);
...
h10=colorbar('Location','eastoutside','Position',pos10,'FontSize',16);
h10.Ticks = [-0.06 -0.03 0 0.03 0.06];
h10.TickLabels = {'-0.06','-0.03','0','0.03','0.06'};
%xlabel('Longitude')
xlabel('Longitude')
ylabel('Pressure (hPa)')
xticks([75.6250 95.6250 115.6250])
...
yticks([600 700 800 900])
yticklabels({'600','700','800','900'})
set(get(h10,'Title') ,'String','m^{2}/s^{2}');
title({'(h) SGS TKE (East Asia; 20N-40N)','(AM4-EDMF-HET minus AM4-EDMF)'},'FontSize',14);

%-----
cd('/home/Khaled.Ghannam/awg/xanadu_2022.08/james_ana_03152023/figures/')
saveas(gcf,'TKE_han_jja_landonly_nolm_12302024.png')
