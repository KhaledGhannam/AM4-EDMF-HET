...
f_orig = '/archive/Khaled.Ghannam/awg/xanadu_2022.08/concatenation/1980_2014_am4/netrad_toa.nc';
f_ncep = '/archive/Khaled.Ghannam/awg/xanadu_2022.08/james_data_03152023/1980_2014_ncep_zas/netrad_toa.nc';
f_clasp = '/archive/Khaled.Ghannam/awg/xanadu_2022.08/james_data_03152023/1980_2014_clasp_lnd_disp_zas/netrad_toa.nc';
...
am4_orig.netrad_toa=squeeze(mean(ncread(f_orig, 'netrad_toa'),3));
am4_ncep.netrad_toa=squeeze(mean(ncread(f_ncep, 'netrad_toa'),3));
am4_clasp.netrad_toa=squeeze(mean(ncread(f_clasp, 'netrad_toa'),3));

load('/archive/Khaled.Ghannam/awg/xanadu_2022.08/concatenation/matlab/all_data.mat',...
    'ceres_inp');

%% Set do_print
sprintf('data read successfully, now plotting')
fpath='./'; expn='am4';
...
z2 = am4_orig.netrad_toa';
z3 = am4_ncep.netrad_toa';
z4 = am4_clasp.netrad_toa';
...
framem;
contourfm(lat,lon,a,[-100:10:100], 'LineStyle', 'none'); hold on;
contourm(latx,lonx,lm,'k','LineWidth',1);
box off; axis off;
caxis([-100 100]);
colormap(ax1,cmap);
h1=colorbar('Location','eastoutside','Position',pos1,'FontSize',16);
h1.Ticks = [-100 -50 0 50 100];
h1.TickLabels = {'-100','-50','0','50','100'};
set(get(h1,'Title') ,'String','W/m^{2}');
title({'(a) NETRAD-TOA ; CERES-EBAF', sprintf('(%s; MEAN=%5.1f)',upper(sea),s.mean1)},'FontSize',14);
%-------------------------------------
...
saveas(gcf,'netradtoa_annualmean_nolm.png')
