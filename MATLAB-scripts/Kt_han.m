...
per='%';
% ---- read variables -----
tdtvdif_ncep = squeeze(ncread('/archive/Khaled.Ghannam/awg/xanadu_2022.08/james_data_03152023/1980_2014_ncep_zas/Kt_han_lev.nc','Kt_han'));

tdtvdif_clasp = squeeze(ncread('/archive/Khaled.Ghannam/awg/xanadu_2022.08/james_data_03152023/1980_2014_clasp_lnd_disp_lm_zas/Kt_han_lev.nc','Kt_han'));

pfull_ncep = squeeze(ncread('/archive/Khaled.Ghannam/awg/xanadu_2022.08/james_data_03152023/1980_2014_ncep_zas/Kt_han_lev.nc','phalf'));

pfull_clasp = squeeze(ncread('/archive/Khaled.Ghannam/awg/xanadu_2022.08/james_data_03152023/1980_2014_clasp_lnd_disp_lm_zas/Kt_han_lev.nc','phalf'));
...
"ShowText",true,"LabelFormat","%0.0f", "FaceAlpha",1,'LabelSpacing',300);
...
h3.Ticks = [0 20 40 60 80];
h3.TickLabels = {'0','20','40','60','80'};
...
title({'(a) K_{\phi} (CONUS; 25N-50N)','(AM4-EDMF-HET)'},'FontSize',14);
...
h4.Ticks = [-40 -20 0 20 40];
h4.TickLabels = {'-40','-20','0','20','40'};
%xlabel('Longitude')
ylabel('Pressure (hPa)')
xticks([241.8750 260.6250 279.3750])
xticklabels({'120^{\circ}W','100^{\circ}W','80^{\circ}W'})
...
title({'(b) K_{\phi} (CONUS; 25N-50N)','(AM4-EDMF-HET minus AM4-EDMF)'},'FontSize',14);
...
h5.Ticks = [0 10 20 30 40];
h5.TickLabels = {'0','10','20','30','40'};
...
title({'(c) K_{\phi} (Equatorial Africa; 10S-10N)','(AM4-EDMF-HET)'},'FontSize',14);
...
title({'(d) K_{\phi} (Equatorial Africa; 10S-10N)','(AM4-EDMF-HET minus AM4-EDMF)'},'FontSize',14);
...
contourf(lon(id1),flipud(pfull_clasp(end-max_h:end)),a', 5, 'LineStyle', '-',...
"ShowText",true,"LabelFormat","%0.0f", "FaceAlpha",1,'LabelSpacing',200);
...
yticks([600 700 800 900])
yticklabels({'600','700','800','900'})
set(get(h7,'Title') ,'String','m^{2}/s');
title({'(e) K_{\phi} (Amazon; 10S-10N)','(AM4-EDMF-HET)'},'FontSize',14);
...
h8.Ticks = [-10 -5 0 5 10];
h8.TickLabels = {'-10','-5','0','5','10'};
%xlabel('Longitude')
ylabel('Pressure (hPa)')
xticks([281.8750 300.6250 319.3750])
xticklabels({'80^{\circ}W','60^{\circ}W','40^{\circ}W'})
yticks([600 700 800 900])
yticklabels({'600','700','800','900'})
set(get(h8,'Title') ,'String','m^{2}/s');
title({'(f) K_{\phi} (Amazon; 10S-10N)','(AM4-EDMF-HET minus AM4-EDMF)'},'FontSize',14);
...
h9.Ticks = [0 10 20 30 40];
h9.TickLabels = {'0','10','20','30','40'};
%xlim([75 115])
...
xlabel('Longitude')
ylabel('Pressure (hPa)')
set(get(h9,'Title') ,'String','m^{2}/s');
title({'(g) K_{\phi} (East Asia; 20N-40N)','(AM4-EDMF-HET)'},'FontSize',14);
...
h10.Ticks = [-30 -15 0 15 30];
h10.TickLabels = {'-30','-15','0','15','30'};
xlabel('Longitude')
ylabel('Pressure (hPa)')
xticks([75.6250 95.6250 115.6250])
xticklabels({'75^{\circ}E','95^{\circ}E','115^{\circ}E'})
yticks([600 700 800 900])
yticklabels({'600','700','800','900'})
set(get(h10,'Title') ,'String','m^{2}/s');
title({'(h) K_{\phi} (East Asia; 20N-40N)','(AM4-EDMF-HET minus AM4-EDMF)'},'FontSize',14);

%-----
cd('/home/Khaled.Ghannam/awg/xanadu_2022.08/james_ana_03152023/figures/')
saveas(gcf,'Kt_han_jja_landonly_12302024_revised.png')
