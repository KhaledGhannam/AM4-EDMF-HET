...
ax1=subplot('Position',p1); clear a; a=rn_clasp;
axesm('MapProjection',domap,'origin',[0,180]);
framem;
contourfm(lat,lon,a,[-30:5:150], 'LineStyle', 'none'); hold on;
contourm(latx,lonx,lm,'k','LineWidth',1);
box off; axis off;
caxis([-30 150]);
colormap(ax1,cmap);
h1=colorbar('Location','eastoutside','Position',pos1,'FontSize',16); cbfreeze(h1,cmap);
h1.Ticks = [-30 0 30 60 90 120 150];
h1.TickLabels = {'-30','0','30','60','90','120','150'};
set(get(h1,'Title') ,'String','W/m^{2}');
title({'(a) R_{n} (AM4-EDMF-HET)',sprintf('(%s; MEAN=%5.1f)',sea, s.mean1)},...
'FontSize',14);
...
title({'(b) R_{n} (AM4-EDMF-HET minus AM4-Lock)',sprintf('(DIFF=%5.1f)',s.bias31)},...
'FontSize',14);
...
title({'(c) R_{n} (AM4-EDMF-HET minus AM4-EDMF)',sprintf('(DIFF=%5.1f)',s.bias32)},...
'FontSize',14);
...
saveas(gcf,'netradiation_annualmean.png')
