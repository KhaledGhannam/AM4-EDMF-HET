...
zpbl_clasp_jja_diur = squeeze(mean(zpbl_clasp2y(:,:,:,6:8),4,'omitnan'));
zpbl_clasp_jja_diur (land_frac_diu<0.4)=NaN;

zpbl_claspnolm_jja_diur = squeeze(mean(zpbl_claspnolm2y(:,:,:,6:8),4,'omitnan'));
zpbl_claspnolm_jja_diur (land_frac_diu<0.4)=NaN;

zpbl_ncep_jja_diur = squeeze(mean(zpbl_ncep2y(:,:,:,6:8),4,'omitnan'));
zpbl_ncep_jja_diur (land_frac_diu<0.4)=NaN;

zpbl_am4_jja_diur = squeeze(mean(zpbl_am42y(:,:,:,6:8),4,'omitnan'));
zpbl_am4_jja_diur (land_frac_diu<0.4)=NaN;

zpbl_clasp_djf_diur = squeeze(mean(zpbl_clasp2y(:,:,:,1:2),4,'omitnan'));
zpbl_claspnolm_djf_diur = squeeze(mean(zpbl_claspnolm2y(:,:,:,1:2),4,'omitnan'));
zpbl_ncep_djf_diur = squeeze(mean(zpbl_ncep2y(:,:,:,1:2),4,'omitnan'));
zpbl_am4_djf_diur = squeeze(mean(zpbl_am42y(:,:,:,1:2),4,'omitnan'));
...
ax1=subplot('Position',p1); clear a; a=zpbl_clasp_jja;
...
ax2=subplot('Position',p2); clear a; a=zpbl_clasp_jja - zpbl_ncep_jja;
axesm('MapProjection',domap,'origin',[0,180]);
framem;
...
ax3=subplot('Position',p3); clear a; a=zpbl_clasp_jja - zpbl_claspnolm_jja;
axesm('MapProjection',domap,'origin',[0,180]);
framem;
contourfm(lat,lon,a,[-100:5:100], 'LineStyle', 'none'); hold on;
contourm(latx,lonx,lm,'k','LineWidth',1);
...
set(hh,'Orientation','Vertical','Position',[0.4501437593020475 0.53299366974578 0.0531525369942488 0.0574509810653386],...
'box','off','FontSize',16)
...
ax8.FontSize = 16;
xlabel('Time (LST)','Interpreter','latex','FontSize',16)
title('(h) Equatorial Africa','FontSize',14)
set(gca,'XMinorTick','on','YMinorTick','on')
set(gca,'TickDir','out');
...
saveas(gcf,'zpbl_diurnal_map_jja_revised.png')
